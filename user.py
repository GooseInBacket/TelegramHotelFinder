...
class User:
    ...
    Methods:
         ...
         | get_amount(user_id) - возвращает кол-во результатов, которые необходимо вернуть
         |  пользователю
         | get_photo(user_id) - возвращает ответ пользователя на вопрос "нужно ли ему фото"
         | set_user(user_id, current_command) - создать нового пользователя
         | remove_user(user_id) - удалить пользователя
         | well_done(user_id) - полное, удачное выполнение команды
